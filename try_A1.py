...
TTS_CONFIG = {
    ...
    "spd": 5,  # 语速（0-9）- 可调参数
    "pit": 5,  # 语调（0-9）- 可调参数
    "daily_char_limit": 999000  # 配额修改为999000字
}

tts_client = AipSpeech(
    TTS_CONFIG["app_id"],
    TTS_CONFIG["api_key"],
    TTS_CONFIG["secret_key"]
)

CHAR_COUNT_FILE = "tts_char_count.txt"
VAD_CONFIG = {
    "mode": 2,  # VAD灵敏度（0-3，3最灵敏）- 可调参数
    "sample_rate": 16000,  # 录音采样率（固定16k，适配百度ASR）
    "frame_duration_ms": 30,  # 帧时长（固定30ms，webrtcvad要求）
    "silence_threshold": 100,  # 录音静音停止阈值（3秒）- 可调参数
    "loud_sound_frames": 5,  # 大声音中断阈值（扩大调整范围）- 可调参数
    "min_loud_frames": 1,  # 最小中断帧数（1帧=30ms）
    "max_loud_frames": 30  # 最大中断帧数（30帧=900ms）
}
...
DEFAULT_MIC = None  # 后续通过健壮逻辑获取
input_mode = "voice"  # 输入模式："voice" 语音输入，"text" 文字输入
mode_lock = threading.Lock()  # 用于线程安全地切换模式
ai_background = ""  # AI背景设定，默认为空
show_control_panel = False  # 控制面板显示标记
wake_word = "你好"  # 默认唤醒词


# --------------------------- 输入模式与控制面板快捷键切换功能 ---------------------------
def setup_mode_switch_listener():
    """设置键盘监听器，用于切换输入模式和打开控制面板"""
    ...
    keyboard.add_hotkey('alt', on_alt_press)  # 添加Alt键监听
...
def create_control_panel():
    ...
    current_params = {
        ...
        "ai_background": ai_background,  # 保存当前AI背景设定
        "wake_word": wake_word  # 保存当前唤醒词
    }
    ...
    # 4. 定义取消函数（不更新参数）
    def cancel_params():
        root.destroy()  # 直接关闭窗口，不修改任何原有参数

    # 5. 面板布局（新增唤醒词设置部分）
    # --------------------------- 5.0 唤醒词设置框架 ---------------------------
    wake_word_frame = ttk.LabelFrame(root, text="唤醒词设置", padding=(10, 5))
    ...
    wake_word_entry.insert(0, current_params["wake_word"])  # 填充当前唤醒词

    ttk.Label(wake_word_frame, text="提示：设置后立即生效，无需重启", foreground="gray").grid(
        row=1, column=0, columnspan=2, sticky=tk.W, pady=(0, 5))

    # --------------------------- 5.1 AI背景设定框架 ---------------------------
    ai_frame = ttk.LabelFrame(root, text="AI背景设定", padding=(10, 5))
    ai_frame.pack(fill=tk.X, padx=20, pady=(10, 5))

    ttk.Label(ai_frame, text="请输入AI的背景设定（角色、性格、知识范围等）：").pack(anchor=tk.W, pady=(5, 10))
    ...
    ttk.Label(ai_frame, text="提示：保存后将重置当前对话并应用新设定", foreground="gray").pack(anchor=tk.W, pady=(0, 5))

    # --------------------------- 5.2 VAD配置框架（扩大中断时长范围） ---------------------------
    vad_frame = ttk.LabelFrame(root, text="VAD语音检测配置", padding=(10, 5))
    vad_frame.pack(fill=tk.X, padx=20, pady=(10, 5))

    # VAD灵敏度（0-3，3最灵敏）
    ttk.Label(vad_frame, text="VAD灵敏度（0=低，3=高）：").grid(row=0, column=0, sticky=tk.W, pady=8)
    vad_mode_slider = ttk.Scale(
        vad_frame, from_=0, to=3, orient=tk.HORIZONTAL, value=current_params["vad_mode"],
        command=lambda v: ttk.Label(vad_frame, text=f"当前：{int(float(v))}").grid(row=0, column=2)
    )
    vad_mode_slider.grid(row=0, column=1, padx=10, sticky=tk.EW)
    ttk.Label(vad_frame, text=f"当前：{current_params['vad_mode']}").grid(row=0, column=2, padx=10)

    # 录音静音停止阈值（50=1.5秒，200=6秒）
    ttk.Label(vad_frame, text="录音静音停止时间（帧，1帧=30ms）：").grid(row=1, column=0, sticky=tk.W, pady=8)
    vad_silence_slider = ttk.Scale(
        vad_frame, from_=50, to=200, orient=tk.HORIZONTAL, value=current_params["vad_silence"],
        command=lambda v: ttk.Label(vad_frame, text=f"当前：{int(float(v))}帧（{int(float(v)) * 0.03:.1f}秒）").grid(row=1,
                                                                                                                  column=2)
    )
    vad_silence_slider.grid(row=1, column=1, padx=10, sticky=tk.EW)
    ttk.Label(vad_frame,
              text=f"当前：{current_params['vad_silence']}帧（{current_params['vad_silence'] * 0.03:.1f}秒）").grid(row=1,
                                                                                                                 column=2,
                                                                                                                 padx=10)

    # 扩大大声音中断阈值范围（1-30帧，30ms-900ms）
    ttk.Label(vad_frame, text="中断播放的声音时长（帧，1帧=30ms）：").grid(row=2, column=0, sticky=tk.W, pady=8)
    vad_loud_slider = ttk.Scale(
        vad_frame,
        from_=VAD_CONFIG["min_loud_frames"],
        to=VAD_CONFIG["max_loud_frames"],
        orient=tk.HORIZONTAL,
        value=current_params["vad_loud"],
        command=lambda v: ttk.Label(vad_frame, text=f"当前：{int(float(v))}帧（{int(float(v)) * 0.03:.1f}秒）").grid(row=2,
                                                                                                                  column=2)
    )
    vad_loud_slider.grid(row=2, column=1, padx=10, sticky=tk.EW)
    ttk.Label(vad_frame, text=f"当前：{current_params['vad_loud']}帧（{current_params['vad_loud'] * 0.03:.1f}秒）").grid(
        row=2, column=2, padx=10)

    ttk.Label(vad_frame, text="提示：值越小越灵敏（易被打断），值越大越迟钝（不易被打断）", foreground="gray").grid(
        row=3, column=0, columnspan=3, sticky=tk.W, pady=(5, 0))

    # --------------------------- 5.3 TTS配置框架 ---------------------------
    tts_frame = ttk.LabelFrame(root, text="TTS语音合成配置", padding=(10, 5))
    tts_frame.pack(fill=tk.X, padx=20, pady=(10, 5))

    # 音量（0-15）
    ttk.Label(tts_frame, text="音量（0=最小，15=最大）：").grid(row=0, column=0, sticky=tk.W, pady=8)
    tts_vol_slider = ttk.Scale(
        tts_frame, from_=0, to=15, orient=tk.HORIZONTAL, value=current_params["tts_vol"],
        command=lambda v: ttk.Label(tts_frame, text=f"当前：{int(float(v))}").grid(row=0, column=2)
    )
    tts_vol_slider.grid(row=0, column=1, padx=10, sticky=tk.EW)
    ttk.Label(tts_frame, text=f"当前：{current_params['tts_vol']}").grid(row=0, column=2, padx=10)

    # 语速（0-9）
    ttk.Label(tts_frame, text="语速（0=最慢，9=最快）：").grid(row=1, column=0, sticky=tk.W, pady=8)
    tts_spd_slider = ttk.Scale(
        tts_frame, from_=0, to=9, orient=tk.HORIZONTAL, value=current_params["tts_spd"],
        command=lambda v: ttk.Label(tts_frame, text=f"当前：{int(float(v))}").grid(row=1, column=2)
    )
    tts_spd_slider.grid(row=1, column=1, padx=10, sticky=tk.EW)
    ttk.Label(tts_frame, text=f"当前：{current_params['tts_spd']}").grid(row=1, column=2, padx=10)

    # 语调（0-9）
    ttk.Label(tts_frame, text="语调（0=最低，9=最高）：").grid(row=2, column=0, sticky=tk.W, pady=8)
    tts_pit_slider = ttk.Scale(
        tts_frame, from_=0, to=9, orient=tk.HORIZONTAL, value=current_params["tts_pit"],
        command=lambda v: ttk.Label(tts_frame, text=f"当前：{int(float(v))}").grid(row=2, column=2)
    )
    tts_pit_slider.grid(row=2, column=1, padx=10, sticky=tk.EW)
    ttk.Label(tts_frame, text=f"当前：{current_params['tts_pit']}").grid(row=2, column=2, padx=10)
    ...
    tts_per_menu = ttk.OptionMenu(tts_frame, tts_per_var, current_params["tts_per"], *tts_per_options.keys(),
                                  command=lambda v: ttk.Label(tts_frame, text=f"当前：{tts_per_options[int(v)]}").grid(
                                      row=3, column=2))
    tts_per_menu.grid(row=3, column=1, padx=10, sticky=tk.EW)
    ttk.Label(tts_frame, text=f"当前：{tts_per_options[current_params['tts_per']]}").grid(row=3, column=2, padx=10)
